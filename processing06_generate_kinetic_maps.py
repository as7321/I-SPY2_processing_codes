...
scan_details_filename = '/home/as7321/Data/I-SPY2_processed/img_resolution_info.xlsx'
scan_details = pd.read_excel(scan_details_filename, sheet_name='scantime')

input_dir_name = '/data/cbig/I-SPY2/shared/post_registration/'
input_mask_dir_name = '/data/cbig/I-SPY2/shared/post_registration_ftv/'
output_dir_name = '/data/cbig/I-SPY2/shared/kineticmaps_resampled_registered_withoutftv/'

contrast_time = ['pre', 'post1', 'post2']
scan_time = ['T0', 'T1']
...
for patient in ptnames:
    print("Processed " + str(count) + " out of 777 patients")
    try:
    ...
    for scan in scan_time:
        f_pre = f_post1 = f_post2 = f_ftv = None
        try:
            os.mkdir(os.path.join(output_dir_name, patient, scan))
        except FileExistsError:
            pass
        ...
        for f in ptfiles:
            ...
            f_post2 = os.path.join(pt_dir_name, f) if scan in f and contrast_time[2] in f else f_post2
        for f in maskfiles:
        ...
        t2 = pt_details.iloc[0]['time_post2']

        images_path = [f_pre, f_post1, f_post2]
        ...
        for k in masks.keys():
            ...
            current_filename = k + '.nii.gz'
            out_file_name = os.path.join(output_dir_name, patient, scan, current_filename)
