...
time_point = "T1" #"T0" or "T1"
covar = "manufacturer" #"manufacturer" or "fieldstrength"
version = "v02"

############# USER INPUT #####################

#establish paths where your spreadsheets (features, clnical, and batch) and code are located
datapath = "/groups/dk3360_gp/projects/I-SPY2/shared/features_harmonized_" + version + "/"
...
resultsfolder_main = "/groups/dk3360_gp/projects/I-SPY2/shared/check_pvalue_after_harmonization/"
if not os.path.exists(resultsfolder_main):
    os.mkdir(resultsfolder_main)


#######################################################################################################333
resultsfolder = os.path.join(resultsfolder_main, covar + "_" + version  +"_c2b2/")
...
for datfile in current_datfile:
    datfile = os.path.splitext(datfile)[0] + ".csv"
    ...
    data_df = pd.read_csv(datapath+datfile)
    data_df.rename(columns={'case':'SubjectID'}, inplace = True)
    print(data_df.head())
